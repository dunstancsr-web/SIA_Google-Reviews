        ...
        for name, (pkl_path, meta_path) in model_files.items():
            ...
            if os.path.exists(meta_path):
                with open(meta_path) as f:
                    m_meta = json.load(f)
                    # Load dual benchmarks (Standard vs LLM-Assisted)
                    ...
                    # Primary Benchmark (LLM-Assisted is the default high-water mark)
...
COLUMN_DEFINITIONS = {
    ...
    "llm_sentiment_score": "LLM-generated sentiment score (-1.0 to 1.0). Note: This is produced by a large language model (Llama-3) reading the same review text. It is highly correlated with the star rating (r=0.87) and serves as an LLM-assisted pre-score, not an independent feature.",
    ...
    with dist_cols[1]:
        ...
        with cap_row_col:
            st.caption("ℹ️ How does the AI 'read' the mood (sentiment)?", help=(
                "**What is this?**\n"
                "A large language model (Llama-3) reads every word the passenger wrote and produces a sentiment score.\n\n"
                "**Data Source Mapping:**\n"
                "- Feature: `llm_sentiment_score` (-1.0 to 1.0)\n"
                "- Correlation with star rating: r = 0.87 (the LLM and rating measure similar constructs)\n\n"
                "**AI Grouping Thresholds:**\n"
                "- 🟢 **Positive:** `llm_sentiment_score` > +0.05\n"
                "- ⚪ **Neutral:** between -0.05 and +0.05\n"
                "- 🔴 **Negative:** `llm_sentiment_score` < -0.05\n\n"
                "**Note:** Since the LLM reads the same review text that determines the rating, this score is strongly correlated with star ratings. It captures nuance (sarcasm, mixed sentiment) that raw stars miss, but is not an independent signal."
            ))
    ...
    if not models:
        ...
        with st.container():
            ...
            if not _use_llm_current:
                current_status = ":blue[●] Standard ML only (fastest, works offline)"
            elif (selected_ai_backend == "groq" and groq_api_ready) or (selected_ai_backend == "auto" and groq_api_ready):
                current_status = ":green[●] LLM-Assisted AI active (Groq cloud)"
            elif selected_ai_backend == "ollama" or (selected_ai_backend == "auto" and not groq_api_ready):
                current_status = ":green[●] LLM-Assisted AI active (local Ollama)"
            else:
                current_status = ":blue[●] Standard ML only (no AI service found)"

            help_text = (
                f"**Current mode:** {current_status}\n\n---\n\n"
                "**When ON (LLM-Assisted):** A large language model reads the review text and generates a "
                "high-nuance sentiment score. This score is fed as a feature into the ML models, "
                "significantly boosting accuracy. **Note:** The LLM is essentially pre-scoring the same "
                "text the model predicts on, so the accuracy lift reflects the LLM doing the heavy lifting.\n\n"
                "**When OFF (Standard ML):** Uses TF-IDF, VADER sentiment, and dealbreaker flags only — "
                "fastest and works offline. This represents the model's own learned patterns without LLM assistance."
            )
            ...
            if st.session_state.get("review_analyzed") and review_input.strip() and rating_models:
                ...
                with st.spinner("🧠 AI is deep-diving into the review... This takes about 10-15 seconds."):
                    ...
                    # If LLM-Assisted mode was requested but failed (AI service down), 
                    ...
                    # LLM-Assisted (Setup D) uses the actual Llama-3 score.
                    pass 
                ...
                with st.expander("📊 View Individual Model Scores & Weights", expanded=False):
                    ...
                    st.info("**What do these columns mean?**\n"
                            ...
                            "- **Test Accuracy (Benchmark):** The performance on the 'final exam' (brand new data). When **LLM-Assisted** mode is enabled, accuracy increases because an LLM pre-scores the review sentiment — the LLM does the heavy lifting of understanding nuance, and the ML models learn to leverage that signal.\n"
                ...
                with st.expander(" ✨ Suggested Response & Next Steps", expanded=False):
                    ...
                    if agent_guidance and (agent_guidance.get("suggested_response") or agent_guidance.get("strategic_steps")):
                        ...
                        if use_llm:
                            ...
                            with col_msg_txt:
                                st.warning("Enable **LLM-Assisted Analysis** to unlock recommendations.")
